...
class  PdfToTextFormData {
	...
	if (!isset ($this->__Properties [$member]))
		warning(new \PdfToText\Exceptions\PdfToTextFormException ("Undefined property \"$member\"."));
	...
	if (isset ($this->__Properties [$member])) {
		...
		// then set individual property values
			...
			if ($value_count < $field_count)
				error(new \PdfToText\Exceptions\PdfToTextFormException ("Not enough value parts specified for the \"$member\" property ($value)."));
			else if ($value_count > $field_count)
				error(new \PdfToText\Exceptions\PdfToTextFormException ("Too much value parts specified for the \"$member\" property ($value)."));

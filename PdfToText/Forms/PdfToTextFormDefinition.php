...
use PdfToText\Forms\PdfToTextFormFieldDefinition;
use PdfToTextObjectBase;
...
class  PdfToTextFormDefinition {
	...
	foreach ($form_definition->attributes() as $attribute_name => $attribute_value) {
		switch (strtolower($attribute_name)) {
			...
			default :
				error(new \PdfToText\Exceptions\PdfToTextFormException ("Invalid attribute \"$attribute_name\" in <form> tag."));
		}
	}
	...
	foreach ($form_definition->children() as $child) {
		...
		switch (strtolower($tag_name)) {
			...
			case    'group' :
				...
				foreach ($child->attributes() as $attribute_name => $attribute_value) {
					switch ($attribute_name) {
						...
						case    'fields' :
							...
							if (!count($items))
								error(new \PdfToText\Exceptions\PdfToTextFormException ("Empty \"fields\" attribute in <group> tag."));
						...
						default :
							error(new \PdfToText\Exceptions\PdfToTextFormException ("Invalid attribute \"$attribute_name\" in <group> tag."));
					}
				}
				...
				if (!$name)
					error(new \PdfToText\Exceptions\PdfToTextFormException ("The \"name\" attribute is mandatory in <group> tag."));
			...
			default :
				error(new \PdfToText\Exceptions\PdfToTextFormException ("Invalid tag <$tag_name> in <form> definition."));
		}
	}
		...
		foreach ($def->Constants as $constant) {
			...
			if (isset ($all_constants [$name])) {
				if ($all_constants [$name] != $value)
					error(new \PdfToText\Exceptions\PdfToTextFormException ("Constant \"$name\" is defined more than once with different values."));
			} else {
	...
	foreach ($this->FieldDefinitions as $def) {
		if (!isset ($this->PdfDefinitions [$def->PdfName]))
			error(new \PdfToText\Exceptions\PdfToTextFormException ("Field \"{$def -> PdfName}\" is not defined in the PDF file."));

		if (isset ($names [$def->Name]))
			error(new \PdfToText\Exceptions\PdfToTextFormException ("Field \"{$def -> Name}\" is defined more than once."));

		$names [$def->Name] = true;

		if (isset ($pdf_names [$def->PdfName]))
			error(new \PdfToText\Exceptions\PdfToTextFormException ("PDF Field \"{$def -> PdfName}\" is referenced more than once."));
		...
		foreach ($def->Constants as $constant) {
			...
			if (isset ($constant_names [$constant_name]) && $constant_names [$constant_name] != $constant ['value'])
				error(new \PdfToText\Exceptions\PdfToTextFormException ("Constant \"$constant_name\" is defined more than once with different values."));
	...
	foreach ($this->Groups as $group) {
		if (isset ($group_names [$group ['name']]))
			error(new \PdfToText\Exceptions\PdfToTextFormException ("Group \"{$group [ 'name' ]}\" is defined more than once."));

		if (isset ($names [$group ['name']]))
			error(new \PdfToText\Exceptions\PdfToTextFormException ("Group \"{$group [ 'name' ]}\" has the same name as an existing field."));

		foreach ($group ['fields'] as $field_name) {
			if (!isset ($names [$field_name]))
				error(new \PdfToText\Exceptions\PdfToTextFormException ("Field \"$field_name\" of group \"{$group [ 'name' ]}\" does not exist."));
		}
	}
}
}
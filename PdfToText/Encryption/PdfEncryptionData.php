...
use PdfToText\Encryption\PdfDecryptionAlgorithm;
use PdfToText;
 ...
 **************************************************************************************************************
 **************************************************************************************************************/
class  PdfEncryptionData extends PdfToText\PdfObjectBase {
	// Encryption modes 
	...
	public function __construct($file_id, $mode, $object_id, $object_data) {
					...
					case    2 :
						if ($user_key_length != 32 && $owner_key_length != 32) {
							if (PdfToText\PdfToText::$DEBUG)
								error(new \PdfToText\Exceptions\PdfToTextDecryptionException ("Invalid user and/or owner key length ($user_key_length/$owner_key_length)", $object_id));
						}
		...
		if ($error_unhandled_version || $error_unhandled_revision) {
			if (PdfToText\PdfToText::$DEBUG)
				error(new \PdfToText\Exceptions\PdfToTextDecryptionException ("Unsupported encryption algorithm version {$this -> AlgorithmVersion} revision {$this -> AlgorithmRevision}.",
					$object_id));
		...
		if ($this->Decrypter === false) {
			if (PdfToText\PdfToText::$DEBUG)
				warning(new \PdfToText\Exceptions\PdfToTextDecryptionException ("Unsupported encryption algorithm #{$this -> EncryptionAlgorithm}, " .
	...
	public static function GetInstance($file_id, $object_id, $object_data) {
		...
		switch (strtolower($object_data_match ['mode'])) {
			...
			default :
				if (self::$DEBUG > 1)
					error(new \PdfToText\Exceptions\PdfToTextDecodingException ("Unhandled encryption mode '{$object_data [ 'mode' ]}'", $object_id));

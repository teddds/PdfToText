...
/***
 * Class PdfTexterEncodingMap
 *  A class for fonts having a character map specified with the /Encoding parameter.
 * @package PdfToText
 */
class  PdfTexterEncodingMap extends PdfToText\Maps\PdfTexterCharacterMap {
	...
	public function __construct($object_id, $definitions, $extra_mappings) {
		...
		if (self::$GlobalEncodings === false) {
			$charset_file = PDFTOTEXT_BASEPATH . '/Maps/adobe-charsets.map';
		...
		for ($i = 0, $item_count = count($items); $i < $item_count; $i++) {
			$item = PdfToText\PdfToText::DecodeRawName(trim($items [$i]));

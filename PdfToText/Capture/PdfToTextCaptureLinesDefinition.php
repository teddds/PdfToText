...
use PdfToText\Capture\PdfToTextCaptureArea;
use PdfToText\Capture\PdfToTextCapturedColumn;
use PdfToText\Capture\PdfToTextCapturedLine;
use PdfToText\Capture\PdfToTextCapturedLines;

/***
 * Class PdfToTextCaptureLinesDefinition
 * @package PdfToText\Capture
 */
class    PdfToTextCaptureLinesDefinition extends \PdfToText\Capture\PdfToTextCaptureShapeDefinition {
	// Column areas
		...
		foreach ($node->children() as $child) {
			...
			switch (strtolower($tag_name)) {
				...
				case    'page' :
					...
					$page_attributes = \PdfToText\Capture\PdfToTextCaptureDefinitions::GetNodeAttributes
					(
						$child,
						array
						(
				...
				// <column> tag :
				case    'column' :
					$column_attributes = \PdfToText\Capture\PdfToTextCaptureDefinitions::GetNodeAttributes
					(
						$child,
						array
						(
					...
					if (!isset ($this->Columns [$column_name])) {
						...
						error(new \PdfToText\Exceptions\PdfToTextCaptureException ("Column \"$column_name\" is defined more than once."));

					break;

				// Other tag : throw an exception
				default :
					error(new \PdfToText\Exceptions\PdfToTextCaptureException ("Invalid tag <$tag_name> found in root tag <rectangle>."));
			}
		}
	}

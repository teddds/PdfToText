...
class  PdfToTextCaptureApplicablePages
	...
	public function SetPageCount($count) {
		...
		foreach ($this->PageRanges as $range) {
			...
			if ($low > $high)
				error(new \PdfToText\Exceptions\PdfToTextCaptureException ("Low value ($low) must be less or equal to high value ($high) " .
			...
			if ($low > $count) {
				warning(new \PdfToText\Exceptions\PdfToTextCaptureException ("Low value ($low) is greater than page count ($count) " .
					"in page range specification \"{$range [0]}..{$range [1]}\"."));
				continue;
			}
	...
	private function __parse_page_numbers($text, $extra_data) {
		...
		foreach ($ranges as $range) {
			...
			switch (count($items)) {
				...
				default :
					error(new \PdfToText\Exceptions\PdfToTextCaptureException ("Invalid page range specification \"$range\"."));
			}
			...
			if (is_string($low) && $this->__check_expression($low) === false)
				error(new \PdfToText\Exceptions\PdfToTextCaptureException ("Invalid expression \"$low\" in page range specification \"$range\"."));

			if (is_string($high) && $this->__check_expression($high) === false)
				error(new \PdfToText\Exceptions\PdfToTextCaptureException ("Invalid expression \"$high\" in page range specification \"$range\"."));

...
use PdfToText\Capture\Dev\PdfToTextCapture;
use PdfToText\Encryption\PdfEncryptionData;
use PdfToText\Images\PdfFaxImage;
use PdfToText\Images\PdfInlinedImage;
use PdfToText\Images\PdfJpegImage;
use PdfToText\Maps\Adobe\PdfTexterAdobeMap;
use PdfToText\Maps\PdfTexterCharacterMap;
use PdfToText\PdfTexterFontTable;
use PdfToText\PdfTexterPageMap;
use PdfToText\Capture\PdfToTextCaptureDefinitions;
use PdfToText\Forms\PdftoTextFormDefinitions;
...
class  PdfToText extends PdfObjectBase {
	...
	// Font mappings
	/***
	 * @var bool | \PdfToText\PdfTexterFontTable
	 */
	...
	/***
	 * @var bool | \PdfToText\Encryption\PdfEncryptionData
	 */
	...
	protected $FormDataObjects;


	/***
	 * Capture data
	 * @var PdfToTextCaptureDefinitions
	 */
	public $CaptureDefinitions;

	/***
	 * @var PdfToTextCapture
	 */
...
//		'\b ( (BT) | (ET) ) \s+',
	...
	public function __construct($filename = null, $options = self::PDFOPT_NONE, $user_password = false, $owner_password = false) {
		...
		if (!self::$StaticInitialized) {
			...
			self::$CIDTablesDirectory = PDFTOTEXT_BASEPATH . DIRECTORY_SEPARATOR . 'CIDTables';
			self::$FontMetricsDirectory = PDFTOTEXT_BASEPATH . DIRECTORY_SEPARATOR . 'FontMetrics';
	...
	public function Load($filename, $user_password = false, $owner_password = false) {
		...
		if (!preg_match('#^ [^:]+ ://#ix', $filename) && !file_exists($filename))
			error(new  Exceptions\PdfToTextDecodingException ("File \"$filename\" does not exist."));

		// Load its contents
		$contents = @file_get_contents($filename, FILE_BINARY);

		if ($contents === false)
			error(new Exceptions\PdfToTextDecodingException ("Unable to open \"$filename\"."));
	...
	private function __load($filename, $contents, $user_password = false, $owner_password = false) {
		...
		if ($start_offset === false)        // Not a pdf document !
			error(new Exceptions\PdfToTextDecodingException ("File \"$filename\" is not a valid PDF file."));
		...
		$this->FormDataDefinitions = array();
		...
		switch ($preg_error) {
			...
			case  PREG_INTERNAL_ERROR :
				error(new \PdfToText\Exceptions\PdfToTextDecodingException ("PDF object extraction : the preg_match_all() function encountered an internal error."));
				break;

			case  PREG_BACKTRACK_LIMIT_ERROR :
				error(new \PdfToText\Exceptions\PdfToTextDecodingException ("PDF object extraction : backtrack limit reached (you may have to modify the pcre.backtrack_limit " .
					"setting of your PHP.ini file, which is currently set to " . ini_get('pcre.backtrack_limit') . ")."));
				break;

			case  PREG_JIT_STACKLIMIT_ERROR :
				error(new \PdfToText\Exceptions\PdfToTextDecodingException ("PDF object extraction : JIT stack limit reached (you may disable this feature by setting the pcre.jit " .
					"setting of your PHP.ini file to 0)."));
				break;

			case  PREG_RECURSION_LIMIT_ERROR :
				error(new \PdfToText\Exceptions\PdfToTextDecodingException ("PDF object extraction : recursion limit reached (you may have to modify the pcre.recursion_limit " .
					"setting of your PHP.ini file, which is currently set to " . ini_get('pcre.recursion_limit') . ")."));
				break;

			case  PREG_BAD_UTF8_ERROR :
				error(new \PdfToText\Exceptions\PdfToTextDecodingException ("PDF object extraction : bad UTF8 character encountered."));
				break;

			case PREG_BAD_UTF8_OFFSET_ERROR :
				error(new \PdfToText\Exceptions\PdfToTextDecodingException ("PDF object extraction : the specified offset does not start at the beginning of a valid UTF8 codepoint."));
				break;

			default :
				error(new \PdfToText\Exceptions\PdfToTextDecodingException ("PDF object extraction : unkown PREG error #$preg_error"));
				break;
		}
	...
	public function AddAdobeExtraMappings($mappings) {
		...
		foreach ($mappings as $key => $value) {
			...
			if (is_string($value)) {
				...
				if (!$count)
					error(new \PdfToText\Exceptions\PdfToTextException ("Adobe extra mapping \"$key\" has no values."));
				...
				error(new \PdfToText\Exceptions\PdfToTextException ("Invalid value \"$value\" for Adobe extra mapping \"$key\"."));
	...
	public function GetFormData($template = null, $form_index = 0) {
		...
		if ($form_index > count($this->FormDataObjectNumbers))
			error(new \PdfToText\Exceptions\PdfToTextFormException ("Invalid form index #$form_index."));

		$form_data = $this->FormData [$this->FormDataObjectNumbers [$form_index]];

		if ($template) {
			if (!file_exists($template))
				error(new \PdfToText\Exceptions\PdfToTextFormException ("Form data template file \"$template\" not found."));
	...
	public function SetCaptures($xml_file) {
		if (!file_exists($xml_file))
			error(new \PdfToText\Exceptions\PdfToTextException ("File \"$xml_file\" does not exist."));
			...
			case    self::PDF_FLATE_ENCODING :
				...
				if ($decoded_stream_data === false) {
					if ($this->IsEncrypted) {
						...
						if ($decoded_stream_data === false) {
							if (self::$DEBUG > 1)
								warning(new Exceptions\PdfToTextDecodingException ("Unable to decrypt object contents.", $object_id));
						}
					} else if (self::$DEBUG > 1)
						warning(new \PdfToText\Exceptions\PdfToTextDecodingException ("Invalid gzip data.", $object_id));
				}
	...
	protected function DecodeObjectStream($object_id, $object_data) {
		...
		if (preg_match('#[^/] stream ( (\r? \n) | \r ) (?P<stream> .*?) endstream#imsx', $object_data, $stream_match)) {
			...
			if (self::$DEBUG > 1)
				error(new Exceptions\PdfToTextDecodingException ("Found object stream without gzipped data", $object_id));
		...
		if (!preg_match('/^ \s* (?P<series> (\d+ \s* )+ )/x', $decoded_data, $series_match)) {
			if (self::$DEBUG > 1)
				error(new \PdfToText\Exceptions\PdfToTextDecodingException ("Object stream does not start with integer object id/offset pairs.", $object_id));
		...
		if (count($series) % 2) {
			if (self::$DEBUG)
				warning(new \PdfToText\Exceptions\PdfToTextDecodingException ("Object stream should start with an even number of integer values.", $object_id));
			...
			if (!($instruction_count % 100)) {
				...
				if ($this->Options & self::PDFOPT_ENFORCE_GLOBAL_EXECUTION_TIME) {
					...
					if ($now - self::$GlobalExecutionStartTime > self::$MaxGlobalExecutionTime)
						error(new \PdfToText\Exceptions\PdfToTextTimeoutException ("file {$this -> Filename}", true, self::$PhpMaxExecutionTime, self::$MaxGlobalExecutionTime));
				}
				...
				if ($this->Options & self::PDFOPT_ENFORCE_EXECUTION_TIME) {
					...
					if ($now - $this->ExecutionStartTime > $this->MaxExecutionTime)
						error(new \PdfToText\Exceptions\PdfToTextTimeoutException ("file {$this -> Filename}", false, self::$PhpMaxExecutionTime, $this->MaxExecutionTime));
				}
			}
	...
	private function __next_token($page_number, $data, $data_length, $index) {
		...
		$enhanced_statistics = $this->EnhancedStatistics;
		$enhanced_statistics && $this->Statistics ['Distributions'] ['space'];
	...
	protected function GetTrailerInformation($contents, $pdf_objects) {
		...
		if (!isset ($pdf_objects [$encrypt_object_id])) {
			if (self::$DEBUG)
				error(new \PdfToText\Exceptions\PdfToTextDecodingException ("Object #$encrypt_object_id, which should contain encryption data, is missing."));

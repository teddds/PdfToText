 ...
 */
class  PdfInlinedImage extends PdfToText\PdfImage {
	...
	public function __construct($image_data, $width, $height, $bits_per_component, $color_scheme) {
		...
		if (isset (self::$Decoders [$color_scheme])) {
			if (isset (self::$Decoders [$color_scheme] [$bits_per_component]))
				...
				error(new \PdfToText\Exceptions\PdfToTextDecodingException ("No decoding function has been implemented for image objects having the " .
					self::$DecoderNames [$color_scheme] . " color scheme with $bits_per_component bits per color component."));
		} else
			error(new \PdfToText\Exceptions\PdfToTextDecodingException ("Unknown color scheme $color_scheme."));
	...
	public static function CreateInstance($stream_data, $object_data, $autosave) {
		...
		if (stripos($object_data, '/DCTDecode'))
			return (new \PdfToText\Images\PdfJpegImage ($image_data, $autosave));
		...
		switch (strtolower($color_space_name)) {
			...
			default :
				if (PdfToText\PdfToText::$DEBUG)
					warning(new \PdfToText\Exceptions\PdfToTextDecodingException ("Unsupported color space \"$color_space_name\"."));
		...
		switch ($bits_per_component) {
			...
			default :
				if (PdfToText\PdfToText::$DEBUG)
					warning(new \PdfToText\Exceptions\PdfToTextDecodingException ("Unsupported bits per component : $bits_per_component."));
